...
describe Game do
  ...
  describe '#find_winner' do
    it 'should return X if X won' do
      game = Game.new({board: ["X", "X", "X", "", "X", "O", "O", "", "O"]})
      expect(game.winner).to eq("X")
    end

    it 'should return O if O won' do
      game = Game.new({board: ["X", "X", "O", "X", "X", "O", "", "O", "O"]})
    ...
    it 'should return nil if the game is a tie' do
      game = Game.new({board: ["X", "O", "X", "O", "O", "X", "X", "X", "O"]})
      expect(game.winner).to eq(nil)
    end
  end
